    ...
    mutate(
      ...
      sub = case_when(
        ...
        TRUE ~ "none"
         ...
         binary = case_when(
        str_detect(type, "Radfahrerfurt|Knotenpunkt") ~ "1",
        str_detect(type, "Radfahrstreifen") ~ "1",
        str_detect(type, "Schutzstreifen") ~ "0",
        str_detect(type, "Bussonderfahrstreifen") ~ "0",
        str_detect(type, "baulich") ~ "1",
        str_detect(type, "ohne Trennung") ~ "0",
        str_detect(type, "mit Radverkehr frei") ~ "0",
        str_detect(type, "Geh-/Radweg, durch Markierung unterschieden") ~"1"
      )
    )
